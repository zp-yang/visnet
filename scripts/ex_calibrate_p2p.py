...
x_opt = r['x']
cam_att_opt = np.array(x_opt).reshape(-1)
# %%
...
plt.imshow(drawn_img) 
# %%
pose_data = np.hstack([cam_pos, cam_att_opt]).tolist()

calib_data['pose'][cam_name] = pose_data
print(calib_data)

with open(f"{data_dir}/calibration/calib_data.json", 'w') as fs:
    json.dump(calib_data, fs, indent=2)